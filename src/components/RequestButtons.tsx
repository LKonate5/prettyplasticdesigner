...
export function RequestButtons({
  ...
  const requestQuote = async () => {
    ...
    const body = [
      ...
      ...projectLines(product, schedule, design),
      '',
      `Quantities to order (incl. ${Math.round(order.wastePct * 100)}% waste):`,
      ...order.rows.map((r) => `  • ${r.material.name}: ${r.order} tiles`),
      `  Total: ${order.totalOrder} tiles ≈ ${order.areaToOrderM2.toFixed(1)} m², ${order.weightToOrderKg.toFixed(0)} kg`,
      order.boxes !== null ? `  Boxes: ${order.boxes} (at ${order.tilesPerBox}/box)` : '',
      '',
      STR.quoteImageNote,
      `Design link: ${link}`,
      '',
